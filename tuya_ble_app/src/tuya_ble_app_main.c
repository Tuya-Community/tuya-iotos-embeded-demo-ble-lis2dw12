...
static const char auth_key_test[] = "A2sC4Awg7f6t4R6jHd0g3UFlHcvdumYn";
static const char device_id_test[] = "tuya91e0eb7fd69c";
static const uint8_t mac_test[6] = {0x5D,0xAB,0x74,0x4D,0x23,0xDC}; //The actual MAC address is : DC:23:4D:68:3A:91
...
static void tuya_cb_handler(tuya_ble_cb_evt_param_t* event)
{
    ...
    case TUYA_BLE_CB_EVT_CONNECTE_STATUS:
        ...
        //After connecting to Bluetooth, the current device status is reported
        if (event->connect_status == BONDING_CONN) { // 3: BONDING_CONN
        	dp_update_all();
        }
...
void tuya_ble_app_init(void)
{
    ...
    if (!lis2dw12_init()) {
    	TUYA_APP_LOG_INFO("lis2dw12 init error");
    }

    dp_init();
}

unsigned long update_temperature_time = 0;
extern union AXIS_DATA x_axis_mg;
extern union AXIS_DATA y_axis_mg;
extern union AXIS_DATA z_axis_mg;
void app_exe()
{
	if (clock_time_exceed(update_temperature_time, 100 * 1000)) {
		update_temperature_time = clock_time();

		if (lis2dw12_status(DRDY_STATUS)) {
			x_axis_mg.axis_mg = get_lis2dw12_x_mg();
			y_axis_mg.axis_mg = get_lis2dw12_y_mg();
			z_axis_mg.axis_mg = get_lis2dw12_z_mg();
